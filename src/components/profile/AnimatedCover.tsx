...
interface Orb {
  ...
  opacity: number;
  phase: number; phaseSpeed: number; // breathing
}
...
export default function AnimatedCover({ className = '' }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    // Build palette: accent + accent shifted + surface + secondary
    ...
    const draw = () => {
      ...
      // Bottom-edge fade to match page background
      const fadeGrad = ctx.createLinearGradient(0, h * 0.55, 0, h);
      fadeGrad.addColorStop(0, 'rgba(0,0,0,0)');
      fadeGrad.addColorStop(1, bgH + 'f0'); // ~94% opaque bg color
